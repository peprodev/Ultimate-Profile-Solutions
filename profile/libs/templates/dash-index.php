...
$classes = apply_filters("peprofile_body_class", "section-$current_section");
$current_profile_url = home_url($wp->request);

echo "<style>";
echo file_get_contents(plugin_dir_path(__FILE__) . "/css/style.css");
echo "</style>";
wp_enqueue_style("peprofile_theme", plugins_url("/css/style.css", __FILE__), array(), time());
wp_add_inline_style("peprofile_theme", '/*' . PHP_EOL . '* Global CSS @ PeproDev Ultimate Profile Solutions (https://pepro.dev/ups)' . PHP_EOL . '*/' . PHP_EOL . wp_unslash(get_option("{$PeproDevUPS_Profile->activation_status}-css")));
      ...
      <nav class="navbar-sidebar">
        ...
        <script>
          function make_active_li(e) {
            var div = document.querySelectorAll(`[data-ref='<?php echo $t_section;?>']`), i;
            for (i = 0; i < div.length; ++i) { div[i].className = div[i].className + ' active'; }
          }
          window.onload = make_active_li;
          make_active_li();
          setTimeout(function() { make_active_li() }, 500);
          setTimeout(function() { make_active_li() }, 1000);
        </script>
      </nav>
    </div>
  </aside>
...
$custom_js = wp_unslash(get_option("{$PeproDevUPS_Profile->activation_status}-js"));
