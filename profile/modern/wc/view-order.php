...
# @Last modified time: 2022/08/31 18:49:08
# @License: GPLv2
 */
...
$product_found            = false;
$ticket_url               = false;
$print_invoice            = home_url("?invoice-pdf={$order_id}");
$ticktet_id               = get_post_meta($order_id, "retouch_ticket", true);
if ($ticktet_id && !empty($ticktet_id) && get_post($ticktet_id)) {
	$ticket_url = $that->get_profile_page(array(
		"section" => "tickets",
		"manage"  => "add",
		"cpid"    => $ticktet_id,
	));
...
<div class="view-order-container">
	<div class="voc-head">
		...
		<div class="voc-head-right">
			...
			<a class="print-order" target="_blank" href="<?=$print_invoice;?>">
			...
			<a class="go-to-ticket us-btn-style_3 <?=($ticket_url?"":"disabled");?>" href="<?=($ticket_url?$ticket_url:"#");?>"><?=__("Go to Ticket", $that->td);?></a>
		</div>
	</div>
	...
	<div class="voc-subhead">
		...
		<div class="date">
			...
			<span class='vocv'><?=esc_html( $order->get_date_created()->date("(H:m) Y.m.d") );?></span>
		</div>
		<div class="total">
...
foreach ( $order_items as $item_id => $item ) {
	...
	if (has_term("services", 'product_cat', $product->get_id())) $service_found = true;
	if (!has_term("services", 'product_cat', $product->get_id())) $product_found = true;
}
...
<section class="woocommerce-order-details desktop-view">
	<?php if ($service_found): ?>
		<div class="table-responsive for_services">
				...
				<tbody>
					...
					foreach ( $order_items as $item_id => $item ) {
						...
						if ( $show_purchase_note && $purchase_note ){
							...
							<td colspan='6'>".wpautop( do_shortcode( wp_kses_post( $purchase_note ) ) )."</td></tr>";
						}
						?>
					</tbody>
				</table>
	...
	<?php if ($product_found): ?>
		<div class="table-responsive for_products">
			<table class="woocommerce-table woocommerce-table--order shop_table product_details">
				<thead>
					<tr>
				...
				<tbody>
					<?php
					$rtc_license = get_post_meta($order->get_id(), "retouch_license", true);
					$rtc_support = get_post_meta($order->get_id(), "retouch_support", true);
					foreach ( $order_items as $item_id => $item ) {
						$lineitem_support = isset($rtc_license[$item_id])?$rtc_license[$item_id]:"————";
						$lineitem_license = isset($rtc_support[$item_id])?$rtc_support[$item_id]:"————";
						$product = $item->get_product();
		        if (has_term("services", 'product_cat', $product->get_id())) continue;
						$purchase_note = $product ? $product->get_purchase_note() : '';
		        if ( ! apply_filters( 'woocommerce_order_item_visible', true, $item ) ) { return; }
		        ...
		        <tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'woocommerce-table__line-item order_item', $item, $order ) ); ?>">
		          <td class="woocommerce-table__product-table product-name">
								<div class="woocommerce-table__product-table-nameimg">
									<img class="wcitem-thumb" src="<?=get_the_post_thumbnail_url($product->get_id());?>" alt="" />
			            <div class="namepricewrapper">
			            	<?=wp_kses_post( apply_filters( 'woocommerce_order_item_name', $product_permalink ? sprintf( '<a href="%s">%s</a>', $product_permalink, $item->get_name() ) : $item->get_name(), $item, $is_visible ) );?>
										<?=wc_price($order->get_item_subtotal( $item, false, true ), array( 'currency' => $order->get_currency() ) );?>
			            </div>
		            </div>
		          </td>
							<td class="woocommerce-table__product-table product-quantity">
		            ...
		            <span><?=$lineitem_license;?></span>
		  				...
		  				<td class="woocommerce-table__product-table product-download">
								...
								?>
		            <a href="<?=$dllink;?>" class="wc-btn-download <?=$dlbclass;?>"></a>
		          </td>
		        </tr>
		        <?php
						if ( $show_purchase_note && $purchase_note ){
							echo "<tr class='woocommerce-table__product-purchase-note product-purchase-note'>
							<td colspan='5'>".wpautop( do_shortcode( wp_kses_post( $purchase_note ) ) )."</td></tr>";
						}
		      }
					?>
				</tbody>
		...
		<div class="table-responsive for_services">
			<?php
				...
				foreach ( $order_items as $item_id => $item ) {
					...
					<?php
					if ( $show_purchase_note && $purchase_note ){
						echo "<div class='woocommerce-table__product-purchase-note product-purchase-note'>
						".wpautop( do_shortcode( wp_kses_post( $purchase_note ) ) )."</div>";
					}
				}
			?>
		</div>
				...
				foreach ( $order_items as $item_id => $item ) {
					...
					<div class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'woocommerce-line-item order_item', $item, $order ) ); ?>">
						<div class="lineitem-product-name">
							<?php
								switch ($product->get_id()) {
									...
									break;
									default:
										echo "<img src='".get_the_post_thumbnail_url($product->get_id())."' class='wcitem-thumb' />";
									break;
								}
							?>
						</div>
						<div class="item-info-wrapper">
							<div class="total-descr">
								<div class="lineitem-total">
									<div class="lineitem-prop-value"><?=$item->get_name();?></div>
									<div class="lineitem-prop-title"><?=$order->get_formatted_line_subtotal($item);?></div>
								</div>
								<div class="lineitem-fulldesc">
									<div class="lineitem-dl">
										<?php
							...
							<div class="price-qty-dl">
								<div class="lineitem-support">
									<div class="lineitem-prop-title">SUPPORT</div>
									<div class="lineitem-prop-value"><span><?=$lineitem_support;?></span></div>
								</div>
								<div class="lineitem-license">
									<div class="lineitem-prop-title">LICENSE</div>
									<div class="lineitem-prop-value"><?=$lineitem_license;?></div>
								</div>
							</div>
						</div>
					</div>
...
<?php
if ( $show_downloads && !empty($downloads) ) {
	?>
...
if ( $order->get_customer_note() ) : ?>
	<section class="woocommerce-order-details for_cnote">
		<div class="table-responsive for_customer_note">
			<p><strong><?=__("Customer Note:", $that->td);?></strong>
...
?>
<div class="float-bottom-action mobile-view">
	<a class="go-to-ticket us-btn-style_3 <?=($ticket_url?"":"disabled");?>" href="<?=($ticket_url?$ticket_url:"#");?>"><?=__("Go to Ticket", $that->td);?></a>
</div>
<?php
