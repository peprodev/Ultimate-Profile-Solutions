 ...
 * @Last modified time: 2025/05/27 23:42:22
*/
    ...
    public function __construct($full_load=true) {
      ...
      if ($full_load) {
        ...
        if (current_user_can("manage_options") && is_admin() && isset($_GET["test"]) && !empty($_GET["test"])) {
          $test = sanitize_text_field(trim($_GET["test"]));
          $options = (array) get_option($this->setting_key);
          echo "<pre style='text-align: left; direction: ltr; border:1px solid gray; padding: 1rem; overflow: auto;'>". print_r($options,1) ."</pre>";exit;
        }
