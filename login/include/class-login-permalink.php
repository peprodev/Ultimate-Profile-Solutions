 ...
 * @Last modified time: 2025/05/12 20:20:33
 * @Based on: WPS Hide Login - 1.9.17.2
 * @Based on: https://wordpress.org/plugins/wps-hide-login/
*/
...
if (defined('ABSPATH') && ! class_exists('WPS_Hide_Login')) {
	class WPS_Hide_Login {
		...
		public function __construct() {
			...
			if (is_multisite() && ! function_exists('is_plugin_active_for_network') || !function_exists('is_plugin_active')) { require_once(ABSPATH . '/wp-admin/includes/plugin.php'); }
		...
		public function wpmu_options() {
			$out = '';
			$out .= '<h3>' . __('PeproDev Ultimate Profile Solutions', 'peprodev-ups') . '</h3>';
			$out .= '<p>' . __('This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'peprodev-ups') . '</p>';
			$out .= '<table class="form-table">';
			$out .= '<tr valign="top">';
			$out .= '<th scope="row"><label for="whl_page">' . __('Networkwide default', 'peprodev-ups') . '</label></th>';
		...
		public function admin_init() {
			global $pagenow;
			add_settings_section('peprodev-ups-section', 'WPS Hide Login', array($this, 'whl_section_desc'), 'general' );
			add_settings_field('whl_page', '<label for="whl_page">' . __('Login url', 'peprodev-ups') . '</label>', array($this, 'whl_page_input'), 'general', 'peprodev-ups-section' );
		...
		public function whl_section_desc() {
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network($this->basename()) ) {
				echo '<p>' . sprintf(__('To set a networkwide default, go to <a href="%s">Network Settings</a>.', 'peprodev-ups'), network_admin_url('settings.php#whl-page-input')) . '</p>';
			}
		}
		...
		public function admin_notices() {
			global $pagenow;
			if ( ! is_network_admin() && $pagenow === 'options-general.php' && isset($_GET['settings-updated']) ) {
		...
		private function basename() { return plugin_basename(__FILE__); }
		private function use_trailing_slashes() { return ('/' === substr(get_option('permalink_structure'), -1, 1)); }
		private function user_trailingslashit($string) { return $this->use_trailing_slashes() ? trailingslashit($string) : untrailingslashit($string); }
		public static function get_instance() { if (null == self::$instance) { self::$instance = new self; } return self::$instance; }
		public function admin_notices_incompatible() { echo '<div class="error notice is-dismissible"><p>' . __('Please upgrade to the latest version of WordPress to activate', 'peprodev-ups') . ' <strong>' . __('PeproDev Ultimate Profile Solutions', 'peprodev-ups') . '</strong>.</p></div>'; }
		public function admin_notices_plugin_conflict() { echo '<div class="error notice is-dismissible"><p>' . __('PeproDev Ultimate Profile Solutions could not be activated because you already have Rename wp-login.php active. Please uninstall rename wp-login.php to use PeproDev Ultimate Profile Solutions.', 'peprodev-ups') . '</p></div>'; }
		public function activate() { add_option('whl_redirect', '1'); delete_option('whl_admin'); }
		...
		public function welcome_email($value) { return str_replace('wp-login.php', trailingslashit(get_site_option('whl_page', 'login')), $value); }
