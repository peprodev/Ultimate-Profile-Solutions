...
class PeproCoreLoginSlugChangerClass {
	...
	protected function init() {
		...
		if ( is_plugin_active_for_network( 'rename-wp-login/rename-wp-login.php' ) ) {
			// deactivate_plugins( PEPROCORELOGINSLUGCHANGERBASENAMME );
	...
	private function new_login_slug() {
		if ( $slug = get_option( 'whl_page' ) ) {
			return $slug;
		} else if ( ( is_multisite() && is_plugin_active_for_network( PEPROCORELOGINSLUGCHANGERBASENAMME ) && ( $slug = get_site_option( 'whl_page', 'login' ) ) ) ) {
			return $slug;
		} else if ( $slug = 'login' ) {
			return $slug;
		}
	...
	private function new_redirect_slug() {
		if ( $slug = get_option( 'whl_redirect_admin' ) ) {
			return $slug;
		} else if ( ( is_multisite() && is_plugin_active_for_network( PEPROCORELOGINSLUGCHANGERBASENAMME ) && ( $slug = get_site_option( 'whl_redirect_admin', '404' ) ) ) ) {
			return $slug;
		} else if ( $slug = '404' ) {
			return $slug;
		}
