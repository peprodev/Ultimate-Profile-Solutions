...
class peproKavenegarSMS
{
  ...
  public function setting_kavenegar()
  {
    ...
    ?>
      ...
      <div class='col-lg-12 row justify-content-between mb-3 field-opt-kavenegar_username'>
        <div class="col-lg-6 label"><span><?php esc_html_e("API Key","peprodev-ups");?></span><?=$this->external_link("https://panel.kavenegar.com/Client/Setting/Account");?></div>
        <div class="col-lg-6"><input name="kavenegar_username" value="<?=esc_attr($this->kavenegar_username);?>" dir="ltr" class='form-input single-required mr-2' autocomplete="off" type="text" /></div>
      </div>
      <div class='col-lg-12 row justify-content-between mb-3 field-opt-kavenegar_sendernumber'>
        <div class="col-lg-6 label"><span><?php esc_html_e("Sender number","peprodev-ups");?></span><?=$this->external_link("https://panel.kavenegar.com/Client/Setting/Account");?></div>
        <div class="col-lg-6"><input name="kavenegar_sendernumber" value="<?=esc_attr($this->kavenegar_sendernumber);?>" dir="ltr" class='form-input single-required mr-2' autocomplete="off" type="text" /></div>
      </div>
  ...
  public function setting_kavenegar_lookup()
  {
    ...
    ?>
      ...
      <div class='col-lg-12 row justify-content-between mb-3 field-opt-kavenegarlookup_api_key'>
        <div class="col-lg-6 label"><span><?php esc_html_e("API Key","peprodev-ups");?></span><?=$this->external_link("https://panel.kavenegar.com/Client/Setting/Account");?></div>
        <div class="col-lg-6"><input name="kavenegarlookup_api_key" value="<?=esc_attr($this->kavenegarlookup_api_key);?>" dir="ltr" class='form-input single-required mr-2' autocomplete="off" type="text" /></div>
      </div>
      <div class='col-lg-12 row justify-content-between mb-3 field-opt-kavenegarlookup_template_id'>
        <div class="col-lg-6 label"><span><?php esc_html_e("Lookup Template","peprodev-ups");?></span><?=$this->external_link("https://panel.kavenegar.com/client/Verification");?></div>
        <div class="col-lg-6"><textarea name="kavenegarlookup_template_id" autocomplete="off" class='form-input single-required mr-2' placeholder="e.g: token={OTP}template=SendActivationSMS" /><?php echo strip_tags($this->kavenegarlookup_template_id);?></textarea></div>
      </div>
    <?php
    $htmloutput = ob_get_contents();
    ob_end_clean();
  ...
  public function CLASS_KAVENEGAR($numbers=array(), $message="", $otp_code=0)
  {
    $response = false;
    ...
    $message  = str_replace("{OTP}", $otp_code, $message);
    $receiver = implode( ',', (array) $numbers);
    ...
    if ( $response !== true ) {$response = $sms_response;}
    return $response;
  }
  public function CLASS_KAVENEGAR_LOOKUP($numbers=array(), $message="", $otp_code=0)
  {
    $response = false;
