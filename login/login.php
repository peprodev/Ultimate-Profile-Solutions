...
if (!class_exists("PeproDevUPS_Login")){
  class PeproDevUPS_Login
  {
    ...
    public function shortcode__sms_subscription($atts=array(), $content="")
    {
      ...
      <form class="pepro-sms-subscription pss-container <?=$this->form_class;?>" novalidate action="/" method="POST">
    ...
    public function shortcode__smart_btn($atts=array(), $content="")
    {
      ...
      if ( is_user_logged_in() ){
        ...
        echo "[pepro-login-popup trigger='#$uniqid{$trigger}' title='$login_popup_title' reg_title='$register_popup_title' reset_title='$resetpass_popup_title' active='$loggedout_form'] <a id='$uniqid' href='#' class='peprodev-ultimate-profile-solution peprodev-smart-btn logged-out $loggedout_class'>$loggedout_text</a>";
      }
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="pepro-verify-container" id="<?=esc_attr($uniqd);?>" data-pepro-reglogin="<?=esc_attr($uniqd);?>">
                ...
                <form class="pepro-verify inline <?=$this->form_class;?>" id="pepro-verify-inline" method="post">
                  <?php
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="pepro-verify-container" id="<?=esc_attr($uniqd);?>" data-pepro-reglogin="<?=esc_attr($uniqd);?>">
                ...
                <form class="pepro-verify inline <?=$this->form_class;?>" id="pepro-verify-inline-force" method="post">
                  <?php
      ...
      if (wp_doing_ajax() && "pepro_reglogin" === $_POST['action']) {
        ...
        switch (sanitize_text_field($_POST["order"])) {
          ...
          case 'register':
            ...
            // its otp and we have email!
            else{
              if ($this->show_email_field && isset($param["email"])) {
                $param["email"] = strtolower($param["email"]);
                if (!filter_var($param["email"], FILTER_VALIDATE_EMAIL)){
                  $error_array['pepro_dev_login_invalid_email'] = _x("<strong>Error</strong>: The given email address is not valid.", "reg-form-error", "peprodev-ups");
                }
    ...
    public function register_new_user($username=false, $email=false, $mobile=false, $params=array())
    {
      ...
      if (empty(trim($userdata['display_name']))){ $userdata['display_name'] = __("Dear user","peprodev-ups"); }
      if (empty(trim($userdata['first_name']))){ $userdata['first_name']     = __("New user","peprodev-ups"); }
    ...
    public function get_form_register_fields()
    {
      ...
      if ($this->show_email_field){
        ...
        if ($this->is_email_field_req && !$this->login_mobile_otp){
          array_push($login_fields,
            array(
              "meta_name"   => "optverify",
              "type"        => "text",
